...
void main()
{
	...
	char p,f=0;
	char name[3][20]={"swapnil","gaurav","nihar"};
	char pswd[3][20]={"swap29","gau05","nih14"};
	...
	strcpy(m[0].medicneName,"Clear Cold");
	strcpy(m[0].Company,"Combitic");
	strcpy(m[0].info,"Good Medicine for Cold");
	...
	for(p=0;p<5;p++)
	{
		if(strcmp(usrname,name[p])==0)
		{
			printf("Enter Password:\n");
			gets(usrpswd);
			if(strcmp(usrpswd,pswd[p])==0)
			{
